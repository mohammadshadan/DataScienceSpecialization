...
Variation in Length of odontoblasts cells on the basis of Dosage (mg/day) :  
...
Variation in Length of odontoblasts cells on the basis of Delivery Method (OJ or VC) :  
...
Please note, similar results will be reflected by t.test() also.
...
```{r}
ttest0.51 <- t.test(dose0.51$len~dose0.51$dose, paired = FALSE, var.equal = FALSE); 
...
```{r}
ttest0.52 <- t.test(dose0.52$len~dose0.52$dose, paired = FALSE, var.equal = FALSE)
...
```{r}
ttest12 <- t.test(dose12$len~dose0.52$dose, paired = FALSE, var.equal = FALSE)
...
```{r}
ttest0.5 <- t.test(supp0.5$len~supp0.5$supp, paired = FALSE, var.equal = FALSE); 
...
```{r}
ttest1 <- t.test(supp1$len~supp1$supp, paired = FALSE, var.equal = FALSE);
...
```{r}
ttest2 <- t.test(supp2$len~supp2$supp, paired = FALSE, var.equal = FALSE); 
...
Assumptions :  
- Variances are unequal  
- Since tests were done on 60 different Guinea Pigs thus considered not Paired 
- Normal Distribution is considered

Conlcusion based on Exploratory Analysis and Hypothesis Test :  
- Mean length of odontoblasts cells is more when delivery method is Orange Juice compared to Ascorbic Acid  when Dosage of 0.5 and 1 mg/day is given   
...
p
```
Variation in Length on the basis of Dosage (mg/day) :  
- For Dosage of 0.5 and 1, Mean length is more for Orange Juice compared to Ascorbic Acid  
- For Dosage of 2.0, Mean length is almost same for Orange Juice and Ascorbic Acid
...
Variation of Length on the basis of Delivery Method (OJ or VC) :  
- For Orange Juice as well as Ascorbic Acid the Mean length increases as the dosage amount is increased from 0.5 to 2 mg/day
...
```{r,echo=FALSE, results='hide',message=FALSE}
summary(tg$len)
l <- ggplot(tg, aes(x = "",y = len ))
l <- l + geom_boxplot(fill="lightblue")
l <- l + geom_hline(yintercept = mean(tg$len), col="red", linetype = "dashed", size = 1)
l <- l + labs(title = "Figure 1 \n Length of Odontoblasts (cells responsible for tooth growth)")
l
```
```{r, echo=FALSE, results='hide',message=FALSE}
#Using ggplot    
d <- ggplot(tg, aes(x=dose, y=len)) 
d <- d + geom_boxplot(aes(fill=dose))
#d <- d + facet_grid(.~supp)
d <- d + stat_summary(fun.y=mean, geom="point", shape="+", size=7, col = "red") 
d <- d + labs(title = "Figure 1 : Length vs. Dosage \n + represents mean")
d
```

```{r, echo=FALSE, results='hide',message=FALSE}
s <- ggplot(tg, aes(x=supp, y=len)) 
s <- s + geom_boxplot(aes(fill=supp))
s <- s + stat_summary(fun.y=mean, geom="point", shape="+", size=9, col = "red") 
s <- s + labs(title = "Figure 2 : Length vs. Supp \n + represents mean")
s
```
```{r, echo=FALSE, results='hide',message=FALSE}
grid.arrange(d, s, ncol=2)
```
*R Scripts for plotting Figure 1 and Figure 2 are available in APPENDIX (Page 4)
